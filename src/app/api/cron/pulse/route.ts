...
/**
 * The audience fast lane, on a five minute schedule.
 *
 * Page loads already fetch a new reading when the stored one is stale, so on a
 * day when the wall display is on this route mostly finds the work already
 * done. It is the floor for the days it is off, so the week-over-week
